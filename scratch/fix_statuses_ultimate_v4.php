...
foreach ($tenants as $tenant) {
    echo ">>> Harmonisation ULTIME Tenant: {$tenant->nom} ({$tenant->database_name})\n";
    ...
    try {
        ...
        // On cherche une colonne de type "couleur" (color, couleur_bg, couleur)
        $colorFound = false;
        foreach (['color', 'couleur_bg', 'couleur'] as $c) {
            if (in_array($c, $cols)) {
                if ($c !== 'color') {
                    echo "    -> Renommage $c vers color\n";
                    $db->statement("ALTER TABLE etat_document CHANGE $c color VARCHAR(50)");
                }
                $colorFound = true;
                break;
            }
        ...
        if (!$colorFound) {
            echo "    -> Création colonne color\n";
            $db->statement("ALTER TABLE etat_document ADD COLUMN color VARCHAR(50) NULL");
        }

        // Autres colonnes techniques
        if (!in_array('badge_class', $cols)) $db->statement("ALTER TABLE etat_document ADD COLUMN badge_class VARCHAR(100) NULL");
        if (!in_array('is_default', $cols))  $db->statement("ALTER TABLE etat_document ADD COLUMN is_default TINYINT(1) DEFAULT 0");
