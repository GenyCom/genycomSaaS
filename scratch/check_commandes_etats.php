...
foreach ($tenants as $tenant) {
    ...
    if (Schema::connection('tenant')->hasTable('commandes')) {
        $cols = Schema::connection('tenant')->getColumnListing('commandes');
        echo "    Colonnes 'commandes' : " . implode(', ', $cols) . "\n";
    }

    if (Schema::connection('tenant')->hasTable('etat_document')) {
        $etats = DB::connection('tenant')->table('etat_document')
            ->where('type_document', 'commande_fournisseur')
            ->get();
        echo "    États 'commande_fournisseur' :\n";
        foreach ($etats as $e) {
