    ...
    <aside class="sidebar" :class="{ open: sidebarOpen }">
      ...
      <div style="padding: 1rem 1.25rem; border-top: 1px solid var(--border-color);">
        <div class="flex items-center gap-2">
          <div style="width: 34px; height: 34px; background: var(--gradient-primary); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.8rem; color: white; flex-shrink: 0;">
            {{ auth.user?.prenom?.[0] }}{{ auth.user?.nom?.[0] }}
          </div>
          <div style="flex: 1; min-width: 0;">
            <div style="font-size: 0.85rem; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ auth.fullName }}</div>
            <div style="font-size: 0.7rem; color: var(--text-muted);">{{ auth.tenant?.nom }}</div>
          </div>
...
const theme = ref(localStorage.getItem('genycom_theme') || 'dark')
