    ...
    <div class="transactions-tab-container" v-else-if="activeTab === 'transactions'">
      <div class="transactions-header-bar mb-4">
        ...
        <div class="sub-filters">
          ...
          <button 
            class="filter-pill" 
            :class="{ active: subFilter === 'commande' }"
            @click="subFilter = 'commande'"
          >
            Commandes <span class="pill-count">{{ countsByType.commande }}</span>
          </button>
          <button 
            class="filter-pill" 
            :class="{ active: subFilter === 'br' }"
            @click="subFilter = 'br'"
          >
            Bons Réception <span class="pill-count">{{ countsByType.br }}</span>
          </button>
          <button 
            class="filter-pill" 
          ...
          >
            Factures Achat <span class="pill-count">{{ countsByType.facture }}</span>
          </button>
          <button 
            class="filter-pill" 
            :class="{ active: subFilter === 'avoir' }"
            @click="subFilter = 'avoir'"
          >
            Avoirs Fourn. <span class="pill-count">{{ countsByType.avoir }}</span>
          </button>
        </div>
            ...
            <tr v-if="filteredTransactions.length === 0">
              <td colspan="7" class="text-center">
                <div class="empty-state-box">
                  ...
                  <p class="empty-desc">Il n'y a aucun document commercial correspondant à ces critères pour ce fournisseur.</p>
                </div>
              </td>
            </tr>
          </tbody>
...
const subFilter = ref('all') // 'all', 'commande', 'br', 'facture', 'avoir'
...
function exportToCSV() {
  ...
  const prefix = fournisseur.value?.societe || 'fournisseur_transactions'
  const filename = `transactions_fournisseur_${prefix.toLowerCase().replace(/[^a-z0-9]/g, '_')}.csv`
...
function exportToExcel() {
  ...
  link.setAttribute("href", url)
  const prefix = fournisseur.value?.societe || 'fournisseur_transactions'
  const filename = `transactions_fournisseur_${prefix.toLowerCase().replace(/[^a-z0-9]/g, '_')}.xls`
  link.setAttribute("download", filename)
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
...
const allTransactions = computed(() => {
  ...
  // Commandes (bcf)
  if (fournisseur.value?.commandes) {
    fournisseur.value.commandes.forEach(c => {
      list.push({
        id: c.id,
        type: 'commande',
        typeLabel: 'Commande',
  ...
  if (fournisseur.value?.bons_reception) {
    fournisseur.value.bons_reception.forEach(br => {
      const totalHt = (br.lignes || []).reduce((sum, l) => sum + (parseFloat(l.quantite_recue) || 0) * (parseFloat(l.prix_unitaire) || 0), 0)
      const totalTtc = totalHt * 1.20 // 20% standard VAT
      list.push({
        id: br.id,
        type: 'br',
        typeLabel: 'Réception',
        numero: br.numero,
        date: br.date_reception || br.created_at,
        total_ht: totalHt,
        total_ttc: totalTtc,
        etat: br.etat,
        link: `/bons-reception/${br.id}`
      })
    })
  }

  // Factures achats
  if (fournisseur.value?.factures_achats) {
    fournisseur.value.factures_achats.forEach(fa => {
      list.push({
        id: fa.id,
        type: 'facture',
        typeLabel: 'Facture Achat',
        numero: fa.numero,
        date: fa.date_facture || fa.created_at,
        total_ht: fa.montant_ht,
        total_ttc: fa.montant_ttc,
        etat: fa.etat,
        link: `/factures-achats/${fa.id}`
      })
    })
  }

  // Avoirs
  if (fournisseur.value?.avoirs) {
    fournisseur.value.avoirs.forEach(av => {
      list.push({
        id: av.id,
        type: 'avoir',
        typeLabel: 'Avoir Fourn.',
        numero: av.numero,
        date: av.date_avoir || av.created_at,
        total_ht: av.montant_ht,
        total_ttc: av.montant_ttc,
        statut: av.statut,
        link: `/avoirs-fournisseurs/${av.id}`
      })
    })
  }
...
const countsByType = computed(() => {
  return {
    ...
    commande: allTransactions.value.filter(t => t.type === 'commande').length,
    br: allTransactions.value.filter(t => t.type === 'br').length,
...
function getDefaultStatusLabel(t) {
  if (t.type === 'avoir') {
    return t.statut === 'valide' ? 'Validé' : 'Brouillon'
  }
  return 'Brouillon'
}
...
.type-tag.commande { background: #E0F2FE; color: #0284C7; }
.type-tag.br { background: #F3E8FF; color: #7C3AED; }
