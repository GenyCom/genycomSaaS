  ...
  <div class="bcc-list-view">
    ...
    <div class="topbar">
      ...
      <div class="topbar-actions">
        <router-link to="/bons-commande-client/new" class="btn-primary-custom">
    ...
    <div class="hero-header">
      <div class="hero-avatar bcc-theme">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/></svg>
      </div>
      <div class="hero-meta">
        ...
        <p class="hero-sub">Vous gérez actuellement <strong>{{ bccs.length }}</strong> bon(s) de commande.</p>
      </div>
    </div>
          ...
          <tbody>
            <tr v-for="b in filteredBccs" :key="b.id" class="table-row">
              ...
              <td class="client-cell">
                ...
                <div class="origin-tag mt-1" v-if="b.devis">Devis : {{ b.devis.numero }}</div>
              </td>
              <td class="text-right">
              ...
              <td class="text-center">
                <span 
                  ...
                  :style="{ backgroundColor: b.etat.couleur + '20', color: b.etat.couleur, borderColor: b.etat.couleur + '40' }"
                >
                  {{ b.etat.libelle }}
                </span>
                <span v-else class="status-pill status-neutral">BROUILLON</span>
              </td>
              <td class="text-right">
...
async function fetchEtats() {
  try {
    const { data } = await api.get('/parametrage/referentiels/etats?type_document=bcc')
...
.btn-primary-custom {
  ...
  box-shadow: 0 4px 12px rgba(79, 70, 229, 0.25); transition: transform .2s;
}
.btn-primary-custom:hover { transform: translateY(-1px); background: #3730A3; }
...
.origin-tag { font-size: .65rem; font-weight: 800; color: var(--c-muted); background: #F1F5F9; padding: 2px 6px; border-radius: 4px; width: fit-content; display: inline-block; }
...
.status-badge-dynamic { padding: 4px 12px; border-radius: 100px; font-size: .68rem; font-weight: 800; border: 1px solid currentColor; display: inline-block; white-space: nowrap; }
