  ...
  <div class="animate-fade-in">
    <div class="flex items-center justify-between mb-3">
      <div class="flex items-center gap-2">
        <router-link to="/avoirs-clients" class="btn btn-secondary btn-sm">← Retour</router-link>
        <h2 style="font-size:1.1rem; font-weight:600;">{{ isNew ? 'Nouvel Avoir Client' : 'Détail de l\'Avoir ' + form.numero }}</h2>
      </div>
      <div class="flex gap-2">
        <button v-if="!isNew" class="btn btn-secondary">Imprimer PDF</button>
    ...
    <div class="card mb-3">
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
        <div>
          <h3 class="text-sm text-muted mb-2 text-uppercase">Client & Motif</h3>
          ...
          <div class="form-group">
            ...
            <input v-model="form.motif" type="text" class="form-input" placeholder="Ex: Retour marchandise défectueuse" />
          </div>
        </div>
        <div>
           ...
           <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
              ...
              <div class="form-group">
                <label class="form-label">Réf. Facture Origine</label>
                <select class="form-select">
                  <option value="">Aucune</option>
                  <option value="1">FAC-202604-0001</option>
                  <option value="2">FAC-202604-0012</option>
                </select>
              </div>
           </div>
        </div>
    ...
    <div class="card mb-3">
       <div class="flex justify-between items-center mb-2">
         <h3 class="text-sm text-muted text-uppercase">Articles concernés</h3>
...
if (isNew.value) {
  ...
  form.value.numero = 'AVR-2026-0001'
  form.value.client_id = 1
  form.value.motif = 'Remise commerciale exceptionnelle'
  form.value.lignes = [
    { produit_id: 1, designation: 'Prestation de service', quantite: 1, prix_unitaire: 1000, taux_tva: 20 }
  ]
}
...
function save() {
  alert('Avoir Client enregistré avec succès (Mode Démo)')
